...
| 2026-06-09 | Phase 6 (QA) | `assets/brand.css`, `snippets/card-collection.liquid` | **Collection card gradient:** strengthened legibility wash so kicker + title copy is readable over bright photo areas. **Collection card full-card link:** added `<a class="card__overlay-link">` as a dedicated overlay inside `.card` (`position:absolute;inset:0;z-index:4`). Required `display:block` to override base rule `a:empty{display:none}`. Verified via Puppeteer: 614×600px, `pointer-events:auto`, `elementFromPoint` returns overlay at top/center/bottom of card. Live on theme #157501358279. |
| 2026-06-05 | Homepage header redesign | `assets/brand.css`, `layout/theme.liquid` | Overlay header: announcement bar hidden on homepage, header floats over hero (`position: fixed`), transparent + dark-gradient + white logo/nav/icons in overlay state, reverts to warm cream + dark text on scroll via Dawn's `.scrolled-past-header`/`.shopify-section-header-sticky` classes. Added `template-{{ request.page_type }}` to `<body>` class for page-scoped CSS. Pushed to live "Host To Host Launch" theme. |
